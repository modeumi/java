...
<body>
	...
	<div id="bodyfield">
		...
		<%@ include file = "itempage_desk.jsp" %>
		</c:when>
		<c:when test="${sessionScope.page eq 'chair'}">
		<%@ include file = "itempage_chair.jsp" %>
		</c:when>
		<c:when test="${sessionScope.page eq '침대'}">
		<%@ include file = "itempage_bed.jsp" %>
		</c:when>
		<c:when test="${sessionScope.page eq '옷장'}">
		<%@ include file = "itempage_closet.jsp" %>
		</c:when>
		<c:when test="${sessionScope.page eq '선반'}">
		<%@ include file = "itempage_shelf.jsp" %>
		</c:when>		
		<c:otherwise>
		<%@ include file = "itempage_desk.jsp" %>
		</c:otherwise>
